    ...
    for epoch in range(epochs):
        ...
        for batch, (x, y) in enumerate(dataloader):
            ...
            for i in range(len(y)):
                pred=  np.argmax(y_pred.transpose(1, 2)[i].detach().numpy(), axis=0)
                true_y = y[i].detach().numpy()
                if list(pred) != list(true_y): 
                    accs.append(1)
                    print(pred, '\n', true_y)
                else: accs.append(0)

            print({ 'epoch': epoch, 'batch': batch, 'loss': loss.item(), 'acc': np.sum(accs)/len(accs) })


            tb.add_scalar("Loss/train", loss.item(), batch)
            tb.add_scalar("Accuracy/val", np.sum(accs)/len(accs), batch)
            tb.flush()
